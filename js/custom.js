...
$(document).ready(function () {
  ...
  // Change currency symbol when the account currency changes
  $(".calculator--pl #acct-ccyc").change(function () {
    var currencySymbol,
      currency = $(this).val();

    console.log(currency);

    $(".calculator--pl #profit-loss-acc-ccyc").removeAttr("style");
    ...
    switch (currency) {
      ...
      case "CHF":
        currencySymbol = "Fr.";
        $(".calculator--pl #profit-loss-acc-ccyc").width("75px");
        break;
      case "AUD":
        currencySymbol = "$";
        break;
  ...
  //Restrict opening and closing fields to 3 decimal places if the quote currency is JPY
  $("#closing-pricec, #opening-pricec").bind("keyup change", function () {
    ...
    var currencyPair = $("#currency-pairc").val();
    var quoteCurrency = currencyPair.substring(3, 6); //get quote currency
...
select.addEventListener('change', function () {
  ...
  customSelect.querySelector('.custom-option').textContent = selectedOption.textContent;
});
